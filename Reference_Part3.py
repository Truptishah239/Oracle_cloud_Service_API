...
class MyDialog3(tk.simpledialog.Dialog):
    ...
    def body(self, Frame):
        ...
        self.OpportunityID_label = tk.Label(Frame, width=16, text="OpportunityID",fg="white",bg="#eb67ad")
        self.OpportunityID_label.pack()
        self.OpportunityID_box = tk.Entry(Frame, width=25,highlightthickness=2)
        self.OpportunityID_box.config(highlightbackground = "#f0c7dd", highlightcolor= "#8a6679")
        self.OpportunityID_box.pack()
        return Frame

    def ok_pressed(self):
        # print("ok")
        self.reference_ID = self.reference_ID_box.get()
        self.OpportunityID = self.OpportunityID_box.get()
        self.destroy()
